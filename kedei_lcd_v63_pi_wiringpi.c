 ...
 //
 // V6.2 
 // Modified by Tong Zhang for v6.2
...
void lcd_init(void)
{
	...
	// 11  //sleep out
	// EE 02 01 02 01 
	// ED 00 00  9A 9A 9B 9B 00 00 00 00 AE AE 01 
	// 21 // display invertion o
	// 35 00 //Tearing Effect Line ON (35h)
	// 29 // display on
	// E9 01 //eeprom write disable
	// 36 28 //Memory Access Control (36h)
	// 3A 55 /// set pixel format
	// 2A 00 00 01 3F // set column address
	// 2B 00 00 01 E0 // set page address
	// 29 // display on 
	// 2B 00 00 01 3F  
	// 2A 00 00 01 DF 
	// 2C //write memory start
    lcd_cmd(0x00);
    delayms(10);
    lcd_cmd(0x11);
    delayms(150);
    ...
    //ED 00 00  9A 9A 9B 9B 00 00 00 00 AE AE 01 
    lcd_cmd(0xED); lcd_data(0x00); lcd_data(0x00); lcd_data(0x9A); lcd_data(0x9A); 
    lcd_data(0x9B); lcd_data(0x9B); lcd_data(0x00); lcd_data(0x00); lcd_data(0x00);
    lcd_data(0x00); lcd_data(0xAE); lcd_data(0xAE); lcd_data(0x01);
    lcd_cmd(0x21);
    lcd_cmd(0x35); lcd_data(0x00);
    lcd_cmd(0x29);delayms(200); // Display On
    // E9 01 //eeprom write disable
    lcd_cmd(0xE9); lcd_data(0x01);
    // 36 28
    ...
    delayms(30);
    // 2B 00 00 01 3F 
    lcd_cmd(0x2B); lcd_data(0x00); lcd_data(0x00); lcd_data(0x01); lcd_data(0x3F);
    // 2A 00 00 01 DF 
    lcd_cmd(0x2A); lcd_data(0x00); lcd_data(0x00); lcd_data(0x01); lcd_data(0xDF);
    lcd_cmd(0x2C);
